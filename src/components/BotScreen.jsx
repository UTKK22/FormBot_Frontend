...
const FormDisplay = () => {
  ...
  const handleInputSubmit = async () => {
    ...
    setChatHistory((prevHistory) => [
      ...prevHistory,
      {
        ...
        contentType: currentField.label,
      },
    ]);
  ...
  const renderChatContent = (entry, index) => {
    ...
    switch (entry.contentType) {
      case "Image":
        return (
          <div
            className={`${styles.bubblePlacement} ${contentClass} ${labelClass} ${styles.show}`}
            key={index}
          >
  ...
  const shouldDisplayInputField =
    currentIndex >= 0 && currentField && currentField.type !== "bubble";

  return (
    <div
      className={`${styles.formContainer} ${styles[formDetails.background]}`}
    >
      {!isChatStarted && (
        <div className={styles.initialContainer}>
          <div className={styles.initialGreetingUser}>
            <button onClick={handleButtonClick} className={styles.startButton}>
              Hi, Click Here <br/>To Start 
            </button>
          </div>
        </div>
      )}
